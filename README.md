ATM Simulation System – Python Mini Project
📌 Project Overview

The ATM Simulation System is a simple command-line based Python project that simulates basic ATM functionalities such as balance enquiry, cash deposit, withdrawal, PIN change, and transaction history.
This project is ideal for Python beginners to understand functions, conditional statements, loops, and global variables.

🛠️ Features

🔐 User authentication using PIN

💰 Balance enquiry

➕ Cash deposit

➖ Cash withdrawal with balance check

📜 Transaction history tracking

🔄 Change ATM PIN

❌ Secure exit option

🧑‍💻 Technologies Used

Language: Python 3

Concepts:

Functions

Loops

Conditional statements

Lists

Global variables

User input/output

⚙️ How the Program Works

User enters the ATM PIN to log in.

If the PIN is correct, the ATM menu is displayed.

User selects options from the menu:

Check balance

Deposit money

Withdraw money

View transaction history

Change PIN

All transactions are stored in memory during program execution.

User can exit safely from the system.

▶️ How to Run the Program

Make sure Python 3 is installed on your system.

Save the code in a file named atm_simulation.py

Open terminal / command prompt.

Run the program:

python atm_simulation.py


Enter the default PIN:

1212

🔑 Default Account Details

Initial Balance: ₹5000

Default PIN: 1212

📌 Sample Output
Enter your ATM PIN: 1212
Login Successful!

----- ATM MENU -----
1. Balance Enquiry
2. Deposit Cash
3. Withdraw Cash
4. Transaction History
5. Change PIN
6. Exit

🚀 Future Improvements

Input validation for non-numeric values

Multiple user accounts

File-based data storage

Limited PIN attempts

ATM receipt generation

📚 Learning Outcomes

Understand real-world application logic

Improve Python function usage

Learn menu-driven programming

Practice problem-solving skills




